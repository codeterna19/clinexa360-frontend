...
import { LayoutDashboard, FileSpreadsheet, Banknote, BarChart } from 'lucide-react';
...
export default function AccountantLayout() {
  ...
  const navItems = [
    ...
    { name: 'Invoices', path: '/accountant/invoices', icon: FileSpreadsheet, feature: 'Billing' },
    { name: 'Payroll', path: '/accountant/payroll', icon: Banknote, feature: 'Payroll' },
    { name: 'Financial Reports', path: '/accountant/reports', icon: BarChart, feature: 'Reports' },
  ];
  ...
  return (
    <div className="flex h-screen bg-page font-sans text-text-primary">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <Header 
          title={visibleNavItems.find(i => location.pathname === i.path || (i.path !== '/accountant' && location.pathname.startsWith(i.path)))?.name || user?.clinic_id?.name || 'Clinic Portal'} 
          user={user} 
